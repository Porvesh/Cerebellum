    ...
    auto field = [](const char* name, int e, int a) {
        if (e != a) {
            throw std::invalid_argument(
                std::string("policy shape mismatch: ") + name + " compiled as " +
                std::to_string(e) + ", checkpoint says " + std::to_string(a));
        }
    };
...
// inference_delay <= execution_horizon <= refresh_trigger (§15.5). Says which
// bound broke — the two have opposite causes.
inline void check_horizons(int inference_delay, int execution_horizon,
                          int refresh_trigger) {
    if (execution_horizon < inference_delay) {
        throw std::invalid_argument(
            "execution_horizon (" + std::to_string(execution_horizon) +
            ") < inference_delay (" + std::to_string(inference_delay) +
            "): committed actions left unconstrained, so the seam returns");
    }
    if (execution_horizon > refresh_trigger) {
        throw std::invalid_argument(
            "execution_horizon (" + std::to_string(execution_horizon) +
            ") > refresh_trigger (" + std::to_string(refresh_trigger) +
            "): freezing actions the control thread never reaches");
    }
}

// Not checked here: chunk_duration_ms() against kMaxStalenessMs. Every default
// config violates it, so enforcing it would mean a runtime that can't start or a
// quietly edited bound. Asserted in the test, settled by the §11.2 sweep (§15.1).
inline void RuntimeConfig::validate() const {
    ...
    // The model denoises padded and slices down, so RTC's prefix lives there (§4.5).
    if (action_dim > padded_action_dim) {
        throw std::invalid_argument(
            "action_dim (" + std::to_string(action_dim) + ") > padded_action_dim (" +
            std::to_string(padded_action_dim) + ")");
    }
    ...
    // Zeros against absolute positions command motion to the dataset mean (§15.4).
    if (underrun == UnderrunPolicy::Zeros &&
        action_space == ActionSpace::AbsolutePosition) {
        throw std::invalid_argument(
    ...
    if (stitching == Stitching::Rtc) {
        check_horizons(rtc.inference_delay, rtc.execution_horizon, refresh_trigger);
    }
}
