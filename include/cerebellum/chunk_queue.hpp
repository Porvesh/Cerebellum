// The action chunk queue — the only thing between the two threads (spec.md §4.1,
// §4.2, §4.3).
//
// Deliberately NOT an SpscQueue<Action>. Three requirements make a per-action
// ring the wrong shape:
//
//   - push is a chunk of H actions, pop is one action. A per-item ring publishes
//     an incomplete chunk to the consumer, and can't express "replace the queued
//     tail", which is what discard stitching does.
...
//
// So ownership of whole chunks moves through two SPSC rings — ready (producer to
// consumer) and free (consumer to producer) — over a fixed pool, and the action
// cursor lives entirely on the consumer. Emitting an action is a local index
// computation, not a concurrent operation, and stitching happens when the
// consumer accepts a chunk, on data no other thread is touching.
//
// Indexed by ABSOLUTE control step throughout, per §4.5. That is what makes "what
// did I promise for steps s..s+n" a slice, and it makes discarding a stale prefix
// fall out of the indexing rather than needing bookkeeping.
...
struct Chunk {
    ...
    bool covers(std::int64_t step) const {
        return step >= first_step && step < first_step + count;
    }
};
