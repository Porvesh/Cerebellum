...
Arguments parse_arguments(int argc, char **argv) {
    ...
    if (args.ticks <= 0 || args.warmup_inferences < 0 || args.synthetic_inference_ms < 0.0) {
        throw std::invalid_argument(
            "ticks must be positive; warmup and inference "
...
class DelayedGenerator final : public ChunkGenerator {
   public:
    ...
    bool generate(const InferenceRequest &request, Chunk &out) noexcept override {
        ...
        if (request.rtc.present()) {
            const int committed = std::min(request.rtc.inference_delay, request.rtc.prefix_count);
            for (int i = 0; i < committed; ++i) {
                out.model_actions[static_cast<std::size_t>(i)] =
                    request.rtc.prefix[static_cast<std::size_t>(i)];
                out.actions[static_cast<std::size_t>(i)] =
                    request.rtc.prefix[static_cast<std::size_t>(i)];
            }
        }
        return true;
   ...
   public:
    void configure(const WorkerObservationSchema &schema) {
        ...
        for (const WorkerImageSpec &expected : schema.images) {
            ...
            image.pixels.resize(static_cast<std::size_t>(image.channels) * image.height *
                                image.width);
...
std::uint64_t positive_samples(const PercentileRecorder &recorder) {
    return static_cast<std::uint64_t>(
        std::count_if(recorder.samples().begin(), recorder.samples().end(),
                      [](std::int64_t sample) { return sample > 0; }));
}

void write_summary(std::ostream &out, std::string_view name, const Summary &summary, double divisor,
                   bool trailing_comma = true) {
    out << "    \"" << name << "\": {\"min\": " << summary.min_ns / divisor
        << ", \"p50\": " << summary.p50_ns / divisor << ", \"p90\": " << summary.p90_ns / divisor
        << ", \"p99\": " << summary.p99_ns / divisor << ", \"max\": " << summary.max_ns / divisor
        << "}";
    if (trailing_comma) out << ',';
    out << '\n';
}

void report(std::ostream &out, const Arguments &args, RuntimeLoop &loop, CountingSink &sink,
            Nanos wall_time) {
    ...
    out << std::fixed << std::setprecision(6) << "{\n"
        << "  \"runner\": \"" << (args.runner == PythonRunner::Synthetic ? "synthetic" : "smolvla")
        << "\",\n"
        << "  \"stitching\": \"" << (args.stitching == Stitching::Rtc ? "rtc" : "discard")
        << "\",\n"
        << "  \"refresh_policy\": \""
        << (args.refresh_policy == RefreshPolicy::Tail         ? "tail"
            : args.refresh_policy == RefreshPolicy::Continuous ? "continuous"
                                                               : "horizon")
        << "\",\n"
        << "  \"device\": \"" << args.device << "\",\n"
    ...
    if (args.runner == PythonRunner::Synthetic)
        out << args.synthetic_inference_ms;
    else
        out << "null";
    out << ",\n"
    ...
    out << "  },\n"
        << "  \"action_metrics\": {\n";
    write_summary(out, "discard_seam_linf", seam, static_cast<double>(ActionChunkQueue::kSeamScale),
                  false);
    out << "  },\n"
        << "  \"counts\": {\n"
        << "    \"ticks\": " << metrics.ticks << ",\n"
        ...
        << "    \"rtc_inference_overruns\": " << inference.rtc_inference_overruns << ",\n"
        ...
        << "  },\n"
        << "  \"rates_percent\": {\n"
        << "    \"underrun\": " << (metrics.ticks ? 100.0 * metrics.underruns / metrics.ticks : 0.0)
        << ",\n"
    ...
    try {
        ...
        if (args.runner == PythonRunner::Synthetic) {
            ...
            for (int i = 0; i < args.warmup_inferences; ++i) {
                Chunk warmup;
                if (!python->generate(InferenceRequest{0, -1, config.chunk_size, args.stitching},
                                      warmup)) {
